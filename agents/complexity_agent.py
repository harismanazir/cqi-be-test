...
"""Enhanced Complexity Agent with static analysis + LangSmith LLM enrichment"""
...
class ComplexityAgent(BaseLLMAgent):
    """Static analysis + LLM-enhanced complexity analysis"""

    def __init__(self, rag_analyzer=None):
        super().__init__('complexity', rag_analyzer)
        self.prompt_name = "complexity-agent"  # LangSmith Hub prompt name

    def _run_static_analysis(self, code: str, language: str):
        """Perform static analysis checks for complexity"""
        issues = []
        lines = code.split("\n")
        ...
        for i, line in enumerate(lines, start=1):
            if re.match(function_pattern, line):
                if current_func:
                    ...
                    if func_length > 50:
                        issues.append({
                            "title": f"Function '{current_func}' too long",
                            "description": "Function exceeds 50 lines, consider refactoring.",
                            "line_number": func_start,
                            "category": "complexity",
                            "evidence": f"Function length {func_length} lines"
                        })
        ...
        for i, line in enumerate(lines, start=1):
            indent = len(line) - len(line.lstrip(" "))
            if indent > 16:  # More than 4 nested levels
                issues.append({
                    "title": "Excessive nesting",
                    "description": "Code block has deep nesting, consider flattening logic.",
                    "line_number": i,
                    "category": "complexity",
                    "evidence": f"Nesting depth {indent // 4}"
                })
            ...
            if m:
                ...
                if len(params) > 5:
                    issues.append({
                        ...
                        "description": "Functions should not take more than 5 parameters.",
                        "line_number": i,
                        "category": "complexity",
                        "evidence": f"Parameter count {len(params)}"
                    })
        ...
        for i, line in enumerate(lines, start=1):
            if len(line) > 120:
                issues.append({
                    ...
                    "line_number": i,
                    "category": "complexity",
                    "evidence": f"Line length {len(line)}"
                })
...
{{
  ...
  "confidence": 0.85
    ...
    async def analyze(self, code: str, file_path: str, language: str):
        ...
        if not static_issues:
            print(f"[COMPLEXITY] No complexity issues in {file_path}")
            return {
                'agent': 'complexity',
                'language': language,
                'file_path': file_path,
        ...
        # ✅ Inject static issues into code so BaseLLMAgent can handle it
        # enriched_code = (
        #     f"# Static analysis issues detected:\n"
        #     f"{static_issues}\n\n"
        #     f"# Source code:\n{code}"
        # )
